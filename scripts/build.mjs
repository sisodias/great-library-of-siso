...
function agentsPage(works, assemblies, snapshot, sectionWork) {
  ...
  return page({
    ...
    <section class="relationship-map shell" aria-labelledby="map-title"><div class="map-copy">${eyebrow(`Assembly / ${stack?.version || 'unversioned'}`)}<h2 id="map-title">One stack.<br>Four required components.</h2><p>${esc(stack?.outcome || 'The assembly connects intent, execution, coordination, project state, and proof.')}</p><p><a href="${href('docs/agent-stack-model.html')}">Read the source-backed stack model →</a><br><a href="${href('docs/agent-base-module-map.html')}">See the Agent Base module extraction map →</a><br><a href="${href('docs/skills-repository-map.html')}">Open the Skill repository map →</a></p></div><div class="map-stack">${members.map(({ component, work }, i) => {
